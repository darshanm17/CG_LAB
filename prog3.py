...
def drawCuboid(pos,length,width,height,color):
    cuboid=box(pos=vector(*pos),length=length,width=width,height=height,color=color)
    return cuboid
def drawCylinder(pos,radius,height,color):
...
cuboid=drawCuboid((-2,0,0),2,2,2,color.blue)
translate(cuboid,4,0,0)
rotate(cuboid,angle=45,axis=(0,1,0))
scale(cuboid,1.5,1.5,1.5)
cylinder=drawCylinder((2,2,0),1,10,color.red)
